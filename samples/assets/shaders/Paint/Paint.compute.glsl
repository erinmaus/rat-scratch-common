...
restrict readonly buffer rat_RayBuffer
{
	Ray rat_Rays[];
};
...
buffer rat_RayHitInfoBuffer
{
	RayHitInfo rat_RayHitInfo[];
};
...
void computemain()
{
	...
	if (intersectRayTriangle(ray.origin, ray.direction, a, b, c, projection, side, barycentricCoordinates) && side >= 1)
	{
		...
		vec2 textureCoordinate = barycentricToCartesian(barycentricCoordinates, s, t, r);
