...
vec4 effect(vec4 color, Image image, vec2 textureCoordinate, vec2 screenCoordinate)
{
	vec3 lightNormal = normalize(SIMPLE_DIRECTIONAL_LIGHT);
	vec4 lightDotSurface = vec4(vec3(clamp(dot(lightNormal, frag_VertexNormal), 0.5, 1.0)), 1.0);
