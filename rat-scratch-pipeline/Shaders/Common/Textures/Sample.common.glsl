#include "@Pipeline/Common/Buffers/Textures.common.glsl"
...
{
	RatScratchPipelineTexture textureAtlasInfo = rat_Textures[textureIndex];
...
{
	return ratDirectSampleTexture(rat_PipelineGammaCorrectTextureAtlasView, textureIndex, textureCoordinate);
}

vec4 ratSampleLinearTexture(uint textureIndex, vec2 textureCoordinate)
{
	return ratDirectSampleTexture(rat_PipelineLinearTextureAtlasView, textureIndex, textureCoordinate);
}
...
{
	return ratSampleLinearTexture(textureIndex, textureCoordinate).r;
}