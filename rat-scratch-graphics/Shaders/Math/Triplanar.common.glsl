...
vec3 triplanarWeights(vec3 modelNormal, float offset, float exponent) {
  vec3 w = pow(clamp(abs(modelNormal) - vec3(offset), vec3(0.0), vec3(1.0)),
               vec3(exponent));
  return w / (w.x + w.y + w.z);
}

vec4 sampleTriplanar(Image image,
                     TriplanarTextureCoordinates triPlanarTextureCoordinates,
                     vec3 weight, float scale) {
...
vec4 sampleTriplanarArray(
    ArrayImage image, TriplanarTextureCoordinates triPlanarTextureCoordinates,
    vec3 weight, float scale, float layer) {
  vec4 x =
      Texel(image, vec3(triPlanarTextureCoordinates.x * vec2(scale), layer)) *
      weight.x;
  vec4 y =
      Texel(image, vec3(triPlanarTextureCoordinates.y * vec2(scale), layer)) *
      weight.y;
  vec4 z =
      Texel(image, vec3(triPlanarTextureCoordinates.z * vec2(scale), layer)) *
      weight.z;
