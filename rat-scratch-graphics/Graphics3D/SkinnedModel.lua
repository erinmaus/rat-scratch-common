...
function SkinnedModel.validateAnimations(inputAnimations)
	...
	for _, animation in ipairs(inputAnimations) do
		...
		if name ~= "" then
			assert(
				not animationsByName[name],
				"animation with duplicate name: %s",
				name
			)
...
function SkinnedModel:new(name, meshes, skeleton, animations)
	...
	local outputAnimations, outputAnimationsByName =
		SkinnedModel.validateAnimations(animations)
...
function SkinnedModel:getAnimation(key)
	...
	elseif type(key) == "string" then
		assert(
			self.animationsByName[key] ~= nil,
			"no animation with given name: %s",
			key
		)
