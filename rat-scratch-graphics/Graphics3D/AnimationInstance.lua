...
do
	...
	function AnimationInstance:blend(weight, otherAnimationInstance, animation)
		...
		assert(
			weight >= 0,
			"weight must be greater than or equal to zero; got %f",
			weight
		)
		...
		for i, boneInstance in ipairs(self.boneInstances) do
			...
			if
				not (animation and animation:hasBone(boneInstance:getBone()))
			then
			...
			translation:scale(weight, weightedVector):add(
				otherAnimationInstance.boneInstances[i]:getTranslation(),
				otherAnimationInstance.boneInstances[i]:getTranslation()
			)
			rotation:scale(weight, weightedRotation):add(
				otherAnimationInstance.boneInstances[i]:getRotation(),
				otherAnimationInstance.boneInstances[i]:getRotation()
			)
			scale:scale(weight, weightedVector):add(
				otherAnimationInstance.boneInstances[i]:getScale(),
				otherAnimationInstance.boneInstances[i]:getScale()
			)
		end
	end
