...
function Animation.validateChannels(channels)
    ...
    for _, channel in ipairs(channels) do
        assert(outputChannelsByBone[channel:getBone()] == nil, "duplicate animation channel on bone %s", channel:getBone())
...
function Animation:new(name, channels)
    local outputChannels, outputChannelsByBone, duration = Animation.validateChannels(channels)
...
function Animation:getChannel(key)
    if type(key) == "number" then
        assert(self.channels[key] ~= nil, "no keyed properties at index %s", key)
        return self.channels[key]
    else
        assert(self.channelsByBone[key] ~= nil, "no keyed properties associated with bone %s", key:getName())
