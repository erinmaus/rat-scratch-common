...
function Skeleton.validateBones(bones)
    ...
    for i, bone in ipairs(bones) do
        ...
        if name ~= "" then
            assert(not outputBonesByName[name], "bone with name already exists: %s", name)
            outputBonesByName[name] = bone
        end

        assert(not outputBonesByID[bone:getID()], "bone with ID already exists: %d", bone:getID())
    ...
    return outputBones, outputBonesByID, outputBonesByName, outputBoneToIndex, outputBoneChildren
end

--- @param bones RatScratch.Graphics.Graphics3D.Bone[]
function Skeleton:new(bones)
    local outputBones, outputBonesByID, outputBonesByName, outputBoneToIndex, outputBoneChildren = Skeleton.validateBones(bones)
...
function Skeleton:getBoneChild(bone, index)
    ...
    local children = self.boneChildren[bone]
    assert(children, "bone has no children for bone at index: %d", self:getBoneIndex(bone))
