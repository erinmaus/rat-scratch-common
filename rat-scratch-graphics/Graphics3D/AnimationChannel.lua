...
function AnimationChannel.validateKeyedProperties(inputKeyedProperties)
	...
	for _, keyedProperty in ipairs(inputKeyedProperties) do
		...
		assert(
			keyedPropertiesByName[name] == nil,
			"multiple keyed properties of the same name: %s",
			name
		)
...
function AnimationChannel:new(bone, keyedProperties)
	local outputKeyedProperties, outputKeyedPropertiesByName, duration =
		AnimationChannel.validateKeyedProperties(keyedProperties)
...
function AnimationChannel:getKeyedProperty(key)
	if type(key) == "number" then
		assert(
			self.keyedProperties[key] ~= nil,
			"no keyed properties at index: %d",
			key
		)
		return self.keyedProperties[key]
	elseif type(key) == "string" then
		assert(
			self.keyedPropertiesByName[key] ~= nil,
			"no keyed properties with property name: %s",
			key
		)
