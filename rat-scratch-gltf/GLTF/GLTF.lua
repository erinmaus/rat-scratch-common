...
function GLTF.parseGLBHeader(file)
    ...
    assert(headerBytesRead == 8, "file too small for GLB; only read %d bytes after header", headerBytesRead)

    local version = love.data.unpack("<I4I4", header)
    assert(version == GLTFTypes.GLB_VERSION, "GLB version mismatch; expected %d, got %d", GLTFTypes.GLB_VERSION, version)
end
...
function GLTF.parseGLBBody(file)
    ...
    while true do
        ...
        elseif chunkType == "bin" then
            assert(binaryData == nil, "multiple binary data chunks found in GLB")
...
function GLTF.loadJSONFromFile(filename, file)
    ...
    assert(root and root.asset and root.asset.version == GLTFTypes.GLTF_VERSION, "GLTF version mismatch in JSON; expected %s", GLTFTypes.GLTF_VERSION)
