...
#include "@/Math/CatmullRom.common.glsl"
...
void evaluateCurve(float t, uint tileIndex, out vec2 farVertex, out vec2 previousVertex, out vec2 currentVertex,
				   out vec2 nextVertex, out float smoothness, out float s)
{
	...
	RatScratchWarpedMeshCurveInfo curveInfo = rat_WarpedMeshCurves[curveSubsectionInfo.curveIndex];
	...
	uint stopIndex = min(startIndex + curveSubsectionInfo.count, curveInfo.offset + curveInfo.count);
	...
	uint farVertexIndex = getCurveVertexIndex(k - 1, offset, curveVertexCount, curveClosed);
	uint previousVertexIndex = getCurveVertexIndex(k, offset, curveVertexCount, curveClosed);
	uint currentVertexIndex = getCurveVertexIndex(k + 1, offset, curveVertexCount, curveClosed);
	uint nextVertexIndex = getCurveVertexIndex(k + 2, offset, curveVertexCount, curveClosed);
...
void transformByCurve(uint tileIndex, vec2 bounds, inout vec4 position, inout vec4 normal)
{
	float left = bounds.x;
	...
	float smoothness, s;
	evaluateCurve(t, tileIndex, farVertex, previousVertex, currentVertex, nextVertex, smoothness, s);
	...
	vec2 positionCatmull = evaluateCatmullRom(s, farVertex, previousVertex, currentVertex, nextVertex);
	vec2 tangentCatmull = evaluateCatmullRomTangent(s, farVertex, previousVertex, currentVertex, nextVertex);
	...
	vec2 N = vec2(-T.y, T.x);
	mat3 worldMatrix = mat3(vec3(T.x, 0.0, T.y), vec3(0.0, 1.0, 0.0), vec3(N.x, 0.0, N.y));
...
void computemain()
{
	...
	RatScratchWarpedMeshVertex vertex = rat_WarpedInputMeshVertices[meshInfo.offset + vertexIndex];

	vertex.position = meshInfo.transform * vertex.position;
	vertex.normal.xyz = transpose(inverse(mat3(meshInfo.transform))) * vertex.normal.xyz;
